(function (global) {
  // Simple event tracker for emulator events
  var _eventLog = [];
  var _eventListeners = [];
  ...
  function connectEvent(event, func) {
    if (_lestEvent) {
      if (_lestEvent.event != _eventLog[_eventLog.length - 1]) {
        _lestEvent = _eventLog[_eventLog.length - 1];
  ...
  function loadFiles(paths, cb, gz) {
    ...
    for (var i = 0; i < paths.length; i = (i + 1) | 0) {
      (function () {
        ...
        xhr.onload = function () {
          if (!gz) results[idx] = new Uint8Array(xhr.response);
          else results[idx] = pako.inflate(new Uint8Array(xhr.response));
  ...
  Halfix.prototype.updateScreen = function () {
    if (this.ctx) this.ctx.putImageData(this.image_data, 0, 0);
  };
   ...
   */
  HardDriveImage.prototype["readCache"] = function (
    id,
    buffer,
    offset,
    length
   ...
   * @param {number} length Number of bytes to read
   */
  HardDriveImage.prototype["writeCache"] = function (
    id,
    buffer,
    offset,
    length
